...
import { databaseUrl } from "./db/database-url";

// drizzle-kit (migration 生成/適用) の設定。コード生成ツールの設定なので、
// 同種の orval.config.ts と揃えてリポジトリ直下に置く。
// schema / out のパスはコマンド実行時の CWD (リポジトリルート) 基準で解決される点に注意。
